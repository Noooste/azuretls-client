...
func TestHeaderWithinSession(t *testing.T) {
	session := azuretls.NewSession()
	defer session.Close()

	session.HeaderOrder = []string{"cache-control", "sec-ch-ua", "sec-ch-ua-mobile", "sec-ch-ua-platform", "upgrade-insecure-requests", "user-agent", "accept", "sec-fetch-site", "sec-fetch-mode", "sec-fetch-user", "sec-fetch-dest", "accept-encoding", "accept-language"}
	session.Header = http.Header{
	...
	var parsedResponse scrapeResponse

	if err = response.JSON(&parsedResponse); err != nil {
		t.Fatal(err)
	}

	if !checkOrder(parsedResponse, session.HeaderOrder) {
		t.Fatal("TestHeader failed, headers order is not as expected")
	}
}

func TestHeaderInParam(t *testing.T) {
	session := azuretls.NewSession()
	defer session.Close()
	...
	response, err := session.Get("https://tools.scrapfly.io/api/fp/anything", headers)
...
func TestHeaderAndOrderInParam(t *testing.T) {
	session := azuretls.NewSession()
	defer session.Close()

	order := azuretls.HeaderOrder{"cache-control", "sec-ch-ua", "sec-ch-ua-mobile", "sec-ch-ua-platform", "upgrade-insecure-requests", "user-agent", "accept", "sec-fetch-site", "sec-fetch-mode", "sec-fetch-user", "sec-fetch-dest", "accept-encoding", "accept-language"}
	headers := http.Header{
		"accept":                    {"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7"},
		"accept-encoding":           {"gzip, deflate, br"},
		"accept-language":           {"zh-HK,zh-TW;q=0.9,zh;q=0.8"},
	...
	response, err := session.Get("https://tools.scrapfly.io/api/fp/anything", headers, order)
	...
	if !checkOrder(parsedResponse, order) {
		t.Fatal("TestHeader failed, headers order is not as expected")
	}
}
